...
- **Reconnaissance:** investigate the real execution path and think through the likely implementation. Trace relevant callers, data, configuration, tests, and existing extension points; test assumptions with read-only or otherwise non-mutating checks. If a ticket (Plane, Jira, issue ID) is referenced, fetch and read the original artifact regardless of how detailed the dispatch is. Treat repository evidence and the original artifact as primary context; orchestrator instructions provide the objective and constraints.
