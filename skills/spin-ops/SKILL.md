...
4. After approval, Scout executes and validates under the operations discipline defined in `spin-worker`: idempotent actions, status checks after changes, before/after evidence, and unattended wait/sleep rather than polling loops for asynchronous external jobs.
