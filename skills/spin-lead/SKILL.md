...
- `spin-session` — Create a new child and dispatch the first prompt. `reportBack` defaults to `true` (child result relays back and wakes you); set `false` only for successors or detached sessions. `wake` defaults to `true`; set `false` only when the target must not start a turn.
- `spin-talk` — Send follow-ups to an existing child. Same `reportBack`/`wake` defaults. `envelope: true` wraps the message as an inter-agent child report.
