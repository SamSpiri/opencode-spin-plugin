  ...
  function formatWorkerResult(
    ...
    if (tokens) {
      ...
      if (hardStep > lastSeen) {
        ...
        contextWarning = `Context notice (user guidance): worker context reached tokens(${stepped / 1000}k); output quality degrades at this size. How to proceed is your call — finishing the current step here is fine, but prefer fresh worker sessions for further substantive work. Split remaining work across parallel workers only when a large amount of context is still expected; otherwise hand off sequentially. Brief successors with pointers — open items, decisions, file paths, sessionIds — never pasted file contents; use the relay history you already hold, or let the retiring worker spawn successors and report their sessionIds. A handover file also works when state is complex. Freshly spawned workers may still be busy finishing the retiring worker's last task — spin-talk errors are expected, retry later. This session stays available via spin-talk for quick clarifications.`
      }
    }
    ...
    try {
      ...
      await sendPrompt(sessionID, {
        ...
        text: hard
          ...
          : `Orchestrator notice (user guidance): your session context reached tokens(${stepped / 1000}k); coordination quality degrades at this size. Decide how to finish: either steer the current work to completion without taking on new work or new dispatches, or hand over to a successor orchestrator. Hand over only once every worker is idle — active workers relay results to this session, and handing over mid-dispatch splits control between two orchestrators — unless the user tells you to hand over earlier. Keep a running pointer-based summary — open items, decisions, worker sessionIds, where to look (file paths, not pasted contents) — so the handover fits in a single prompt.`,
      })
    } catch {
