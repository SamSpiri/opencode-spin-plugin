...
- Package published to npm as `opencode-spin`
- Work in the project directory only. Ask approval to modify files in opencode's config directories (`~/.config/opencode/`).

## Install

`npm run install:opencode` - builds and installs the plugin into OpenCode's plugin directory (`~/.config/opencode/plugins/`)
